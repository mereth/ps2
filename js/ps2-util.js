...
.factory('ps2Utils',  function() {
    ...
    var getRankImage = function(rank, faction) {
        var o = ranks[rank];
        if(o) {
            return o[faction + "_image_path"];
        }
        return ranks[100][faction + "_image_path"];
    };

    var getComputedRank = function(experience) {
        experience = experience * 1;
        
        var br100xp = ranks[100].xp_max;
        if (experience <= br100xp) { // 1 to 99
            for (var i = 1; i <= 100; ++i) {
                var rank = ranks[i];
        ...
        var rank = 100;
        var rankupxp = 943447;
        experience = experience - br100xp;
        while (experience >= rankupxp) {
            experience = experience - rankupxp;
            rank = rank + 1;
            if(rankupxp === 943447) {
                rankupxp = 943448;
            }
    ...
    var getFaction = function(factionId) {
